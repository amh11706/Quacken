...
class QuackenPDO extends PDO {
  ...
  public function __construct() {
    ...
    parent::__construct(
      'mysql:dbname=krakenhunt;host=localhost', 'root', '',
      array(
  ...
  function removeGhosts() {
  	$ghosts = $this->query(
     "SELECT UserName, ID FROM $this->lobby
  	...
  	while ($row = $ghosts->fetch()) {
  		...
  		$this->exec(
       "INSERT INTO chat (Name, Message, Type)
  			VALUES ('$u', '< $id', 'note')"
      );
  	}
    return $this;
  ...
  public function getLobbyUser(string $columns) {
    $sth = $this->prepare(
     "SELECT $columns FROM $this->lobby
      WHERE ID = ? AND Token = ? AND IP = '$this->ip' LIMIT 1"
    );
  ...
  public function getNewChat(int $index) {
    return $this->query(
     "SELECT Name, Message, Type FROM chat
      WHERE ID > $index"
    )->fetchAll();
  }
  ...
  public function updateLobbyUser(array $keyValues) {
    ...
    $this->exec(
     "UPDATE $this->lobby SET Timestamp = now(), $sqlString
      WHERE ID = $this->id"
    );
  }
